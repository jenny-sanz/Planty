...
if ( !class_exists('GP_Aloha') ):
	...
	class GP_Aloha
	{
		...
		function output_welcome_page()
		{
			...
			printf( '<div class="gp_aloha %s">', esc_attr($this->options['wrapper_class']) );
			printf( '<h1 class="aloha_header">%s</h1>', esc_html($this->options['page_title']) );
			printf( '<div class="aloha_body">%s</div>', esc_html($content) );
			print( '</div>' );
		}
	}
