...
if(!class_exists("Gold_Plugins_Media_Button")){
	class Gold_Plugins_Media_Button
	{
		...
		function add_media_button($context)
		{
			...
			echo '<div class="gp_media_button_group">';
			$btn = '';
			if (!empty($this->button_dashicon)) {
				$btn = sprintf('<span class="dashicons dashicons-%s wp-media-buttons-icon"></span>', esc_attr($this->button_dashicon));
			}
			printf('<a class="gp_media_button_group_toggle">%s <span class="media_button_label">%s</span> <span class="dashicons dashicons-arrow-down-alt2 wp-media-buttons-icon"></span>	</a>', wp_kses_post($btn), esc_html($this->button_label));
			echo '<ul class="gp_media_button_group_dropdown">';
				...
				foreach($this->buttons as $button) {
					printf($button_template, esc_attr($button['widget_class']), esc_attr($button['shortcode']), esc_attr($button['widget_title']), esc_attr($button['dashicon']), esc_html($button['widget_title']));
				}
			echo '</ul>';
			echo '</div>';			
		}
		...
		function output_media_button_page()
		{
			...
			if ( empty($base) || empty($shortcode) ) {
				echo '<p>' . esc_html__('Error: You must specify a base and a shortcode.') . "</p>\n";
				wp_die();
			}
			...
			<div class="inner_wrapper">
				<div class="editwidget"<?php echo wp_kses_post($width_rule); ?>>
					...
					<form action="#" method="post" onsubmit="gp_insertWidgetIntoPost(); return false;" data-shortcode="<?php echo esc_attr($shortcode); ?>">
						...
						<?php
							if ( is_callable( $control_callback ) ) {
								...
								echo '<p>' . esc_html__('There are no options for this widget.') . "</p>\n";
							}
						 ?>
						</div>
